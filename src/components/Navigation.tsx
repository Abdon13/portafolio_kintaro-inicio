...
export const Navigation = ({tabActiveIndex, setTabActiveIndex}: Props) => {
  return (
    <div className="flex mt-12 border-b border-gray-200">
        {tabs.map(tab => (
            <button className="flex-1 flex justify-center cursor-pointer"
                key={tab.id}
                onClick={() => setTabActiveIndex(tab.id)}
            >
                <p className={`justify-center flex gap-2 items-center border-b-2 pb-1
                ...
                >
                    ...
                    tabActiveIndex == tab.id && tab.name}</span>
                </p>
            </button>
        ))}
    </div>
